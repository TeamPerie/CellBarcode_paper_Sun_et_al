...
# NOTE: output
pdf("./tmp/fig_no_umi_simple_ROC_depth.pdf")
for (i in bc_names(bc_obj_sub)) {
    ...
    x_seq = bc_2df(bc_obj_sub[, i] %>% bc_cure_depth())
...
for (i in bc_names(bc_obj)) {
    simu_file = i
    x_true = d_true_barcode[simu_file == i]
    x_seq = bc_2df(bc_obj[, i] %>% bc_cure_depth())
...
# NOTE: output
write_tsv(d_count_true_barcode, "./tmp/table_count_true_barcode_depth.tsv")
...
# NOTE: output
write_tsv(d_auc, "./tmp/table_auc_no_umi_depth.tsv")
...
#+ eval=F
# NOTE: output
pdf("./tmp/fig_auto_cutoff_point_depth.pdf")
# for (i in seq_along(x_cutoff)[grep("seq_7", names(x_cutoff))]) {
for (i in seq_along(x_cutoff)[1:26]) {
...
write_tsv(d, "./tmp/table_no_umi_auto_threshold_predict_rate_depth.tsv")

#' Apply 0.01 percentage threshold
x_cutoff = lapply(bc_obj@cleanBc, function(x) { x$count %>% sum }) %>% unlist * 0.01
...
#+ eval=F
# NOTE: output
pdf("./tmp/fig_p01_cutoff_point_depth.pdf")
# for (i in seq_along(x_cutoff)[grep("seq_7", names(x_cutoff))]) {
for (i in seq_along(x_cutoff)[1:26]) {
    simu_name = bc_names(bc_obj2)[i]
    ## draw the barcode filtering figures
...
write_tsv(d, "./tmp/table_no_umi_p01_threshold_predict_rate_depth.tsv")


#' Apply 0.001 percentage threshold
x_cutoff = lapply(bc_obj@cleanBc, function(x) { x$count %>% sum }) %>% unlist * 0.001
names(x_cutoff) = rownames(bc_meta(bc_obj))
bc_obj2 = bc_cure_depth(bc_obj, depth = 0, isUpdate=T)
...
#+ eval=F
# NOTE: output
pdf("./tmp/fig_p001_cutoff_point_depth.pdf")
# for (i in seq_along(x_cutoff)[grep("seq_7", names(x_cutoff))]) {
for (i in seq_along(x_cutoff)[1:26]) {
    simu_name = bc_names(bc_obj2)[i]
    ## draw the barcode filtering figures
...
write_tsv(d, "./tmp/table_no_umi_p001_threshold_predict_rate_depth.tsv")
...
#+ eval=F
# NOTE: output
pdf("./tmp/fig_p0001_cutoff_point_depth.pdf")
# for (i in seq_along(x_cutoff)[grep("seq_7", names(x_cutoff))]) {
for (i in seq_along(x_cutoff)[1:26]) {
    simu_name = bc_names(bc_obj2)[i]
    ## draw the barcode filtering figures
...
write_tsv(d, "./tmp/table_no_umi_p0001_threshold_predict_rate_depth.tsv")
...
#+ eval=F
# NOTE: output
pdf("./tmp/fig_p00001_cutoff_point_depth.pdf")
# for (i in seq_along(x_cutoff)[grep("seq_7", names(x_cutoff))]) {
for (i in seq_along(x_cutoff)[1:26]) {
    simu_name = bc_names(bc_obj2)[i]
    ## draw the barcode filtering figures
...
write_tsv(d, "./tmp/table_no_umi_p00001_threshold_predict_rate_depth.tsv")
